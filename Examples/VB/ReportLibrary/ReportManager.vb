...
Public Class ReportManager
    ...
    Private Shared Function CreateReportInfo(t As Type, index As Integer) As ReportInfo
        ...
        Dim attributes As Object() = t.GetCustomAttributes(GetType(DescriptionAttribute), False)

        If attributes.Length > 0 Then
            description = DirectCast(attributes(0), DescriptionAttribute).Description
        End If

        attributes = t.GetCustomAttributes(GetType(DisplayNameAttribute), False)
        Dim reportName As String
        If attributes.Length > 0 Then
            reportName = DirectCast(attributes(0), DisplayNameAttribute).DisplayName
        Else
            reportName = FormatName(t.Name)
        End If

        Dim reportInfo = New ReportInfo(reportName, FormatDescription(description), t, index)
