namespace Telerik.Reporting.Examples.CSharp
{
    ...
    public class CryptoDataResolver
    {
        ...
        static ModelIndexes FillModelIndexer(string headerLine)
        {
            ...
            // remote service occassionally changes the field 'date' with field 'time'. Test which one is available and use it in model.ForDate;
            if (model.ForDate == -1 && model.ForTime > -1)
            {
                model.ForDate = model.ForTime;
            }
