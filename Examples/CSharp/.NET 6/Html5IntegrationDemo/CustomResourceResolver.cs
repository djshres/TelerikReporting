namespace CSharp.Net6.Html5IntegrationDemo
{
    using System;
    using System.IO;
    ...
    /// The CustomResourceResolver class needs to be registered in application's configuration file appsettings.json.
    /// </summary>
    class CustomResourceResolver : IResourceResolver
    {
        ...
        public object Resolve(string resourceUri)
        {
            ...
            {
                System.Diagnostics.Trace.TraceError($"An error occurred when trying to resolve the resource with URI {resourceUri}: {ex.Message}");
        ...
        static string ResolveUri(IResourceResolver resourceResolver, string resourceUri)
        {
            ...
            {
                if (Uri.TryCreate(resolvedUri, UriKind.Absolute, out Uri uri))
                {
        ...
        internal class RawDataResourceResolver : IResourceResolver
        {
            public object Resolve(string resourceUri)
            {
                ...
                {
                    if (Uri.TryCreate(resourceUri, UriKind.Absolute, out Uri uri))
                    {
                        if (!uri.IsFile) // Handle the case when the resourceUri is a URL, like in the ListBoundReport example.
                        {
